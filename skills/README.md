...
The repository installers for Codex and Claude Code copy packages by default and accept `--method symlink` for a live link to the checkout. The selected method is written to the harness-local installed copy of `fpf-settings.toml`, outside the repository, so later installations and updates reuse it automatically. Both modes work on macOS, Linux, and WSL. Copy mode also avoids native Windows symlink-permission requirements.

An explicit user request overrides these embedded defaults for that result. `natural` retains natural FPF language and is the only style where `full`, `short`, or `off` term explanations apply. `general` removes FPF terms from narrative prose in favor of ordinary-language synonyms. `ste` also removes FPF terms and uses simplified wording plus a limited ASD-STE100 Issue 9-inspired overlay: short clear sentences, one topic per sentence, active voice where practical, and vertical lists for complex material. It is guidance only, not a formal-conformance claim. In every style, retain exact FPF locators and source paths in compact evidence or source records rather than narrative prose.
