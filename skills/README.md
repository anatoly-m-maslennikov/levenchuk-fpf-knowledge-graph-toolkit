...
The skills-used section also contains a compact, collapsible FPF source trace. Methodology-consuming skills list every FPF source actually opened exactly once, distinguish files materially used as evidence from files only screened, and report read/used totals. Repository graph files use portable `FPF-Knowledge-Graph/...` paths; non-file-backed editions use stable source identifiers, and renderers without `<details>` show the same list directly. `fpf-route` is the explicit exception: because it uses its embedded skill catalog and makes no methodology claims, it reports that routing basis and marks FPF methodology sources as not applicable instead of showing a misleading zero-count trace.
