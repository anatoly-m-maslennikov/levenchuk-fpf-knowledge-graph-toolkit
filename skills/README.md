...
[`fpf-settings.toml`](fpf-settings.toml) is the build-time source for the suite-wide defaults: `output_style = "general"` and `fpf_terms_explained = "off"`. The generated settings block in every package keeps each standalone copy portable; run `python3 scripts/sync_fpf_skill_settings.py --apply` from the repository root after changing either setting, then run `--check`.
