...
`ParityPlan@Context := ⟨
  ...
  Budgeting?,
  ParityPinSet,
  PlanItemRefs[]?                          // references to [[A_Kernel Architecture Cluster/15_Role-Method-Work Alignment/03_A.15.03 - SlotFillingsPlanItem|A.15.3]] SlotFillingsPlanItem (planned baseline), when parity depends on planned slot fillings
⟩`
