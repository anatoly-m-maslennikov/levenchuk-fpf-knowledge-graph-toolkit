...
> [[A_Kernel Architecture Cluster/06_Signature Stack & Boundary Discipline/06_A.06.01 - U.Mechanism - Reusable Law-Governed Operation Declaration|A.6.1]] governs the semantic content of a `U.Mechanism` declaration. This pattern specialises that content for selection through the exact `EntityOfConcernRef`, effective `U.ReferenceScheme`, direct signature components, SlotSpecs, `OperationAlgebra`, `LawSet`, `AdmissibilityConditions`, and Applicability. An [[F_The Unification Suite (U-Suite)- Concept-Sets, SenseCells & Contextual Role Assignment/09_Alignment and Bridge across Contexts/00_F.09 - Alignment and Bridge across Contexts|F.9]] bridge relation, dated selection `U.Work`, actual `Select` operation application with its `SelectionSlot` binding, any result episteme, [[A_Kernel Architecture Cluster/10_Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph/00_A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]] evidence-provenance graph relation, [[G_Discipline SoTA Patterns Kit/12_11_Telemetry-Driven Refresh and Decay Orchestrator/00_G.11 - Telemetry-Driven Refresh and Decay Orchestrator|G.11]] currentness relation, and any publication relation remain neighboring objects and relations.
...
* **Inputs (conceptual):** admitted candidates; a finite by-value basis of exact upstream binary CPM applications, each with its exact pair, realized `GuardDecision`, and own `ComparisonResultSlot` binding when produced; the exact union of justified relation or poset tokens from those bindings; explicit `CriteriaSlot`, `CNSpecSlot`, `CGSpecSlot`; one `U.ClaimScope` with selected [[A_Kernel Architecture Cluster/02_Role Taxonomy/06_A.02.06 - Unified Scope Mechanism (USM)- Context Slices & Scopes|A.2.6]] `U.ContextSlice` members; the same [[A_Kernel Architecture Cluster/19_CharacteristicSpace & Dynamics Hook (A.CHR-SPACE)/00_A.19 - CharacteristicSpace & Dynamics Hook (A.CHR-SPACE)|A.19]] predicate basis when one governs the comparisons or selection criteria; effective reference plane; explicit evaluation window; and optional TaskSignature and MinimalEvidence policy refs.
...
* **Planned slot fillings:** concrete edition and policy pins are planned fillings under the exact [[A_Kernel Architecture Cluster/15_Role-Method-Work Alignment/03_A.15.03 - SlotFillingsPlanItem|A.15.3]] declaration and are carried by `SlotFillingsPlanItem` rows ([[A_Kernel Architecture Cluster/15_Role-Method-Work Alignment/03_A.15.03 - SlotFillingsPlanItem|A.15.3]] plus `A.19.CHR:4.7.2`). The selector declaration does not bind project-specific fillings. Dated selection `U.Work` remains the performed occurrence; an actual [[A_Kernel Architecture Cluster/06_Signature Stack & Boundary Discipline/06_A.06.01 - U.Mechanism - Reusable Law-Governed Operation Declaration|A.6.1]] `Select` operation application carries effective argument bindings and the selected-set `SelectionSlot` binding; and its [[A_Kernel Architecture Cluster/10_Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph/00_A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]] evidence-provenance path records the evidence and currentness basis used for replay.
...
* **Effective `U.ReferenceScheme`:** the CHR suite reference scheme in which the [[A_Kernel Architecture Cluster/19_CharacteristicSpace & Dynamics Hook (A.CHR-SPACE)/06_A.19.CHR - CHRMechanismSuite|A.19.CHR]] SlotKind lexicon, CN-Spec, CG-Spec, and any current TaskSignature tokens are interpreted.
...
* **Selection-use bindings for each actual application** (required [[A_Kernel Architecture Cluster/06_Signature Stack & Boundary Discipline/06_A.06.01 - U.Mechanism - Reusable Law-Governed Operation Declaration|A.6.1]] occurrence arguments; not CHR SlotKinds and not another container kind):
  ...
  * selected `U.ContextSlice` members under [[A_Kernel Architecture Cluster/02_Role Taxonomy/06_A.02.06 - Unified Scope Mechanism (USM)- Context Slices & Scopes|A.2.6]], without copying membership;
  * the same by-value [[A_Kernel Architecture Cluster/19_CharacteristicSpace & Dynamics Hook (A.CHR-SPACE)/00_A.19 - CharacteristicSpace & Dynamics Hook (A.CHR-SPACE)|A.19]] `CharacteristicSpacePredicate` basis used by the relevant basis members or an explicit `none` when no predicate governs the use;
...
* **LawSet** (minimum): the selection kernel is set-returning and policy-bound
  ...
  6. **No silent boundary change:** `Select` does not silently change candidate universe, comparison-application basis membership, required comparison coverage, any member's pair, eligibility or output binding, selection conditions, [[A_Kernel Architecture Cluster/19_CharacteristicSpace & Dynamics Hook (A.CHR-SPACE)/00_A.19 - CharacteristicSpace & Dynamics Hook (A.CHR-SPACE)|A.19]] predicate basis, claim scope, selected context slices, reference scheme or plane, or evaluation window. A changed binding is another selection application and may require new binary comparisons.
...
* **AdmissibilityConditions** (tri-state guard; fail-closed on missing admissibility, comparison coverage, token provenance, or evidence)
  ...
  * `pass` requires: (i) every basis member's exact pair lies inside `CandidateSetSlot`; (ii) the basis covers every binary comparison required by the candidate universe and explicit selection conditions; (iii) every consumed relation token traces to a member's own output binding; (iv) explicit selection conditions and tie-breakers; (v) compatible [[A_Kernel Architecture Cluster/19_CharacteristicSpace & Dynamics Hook (A.CHR-SPACE)/00_A.19 - CharacteristicSpace & Dynamics Hook (A.CHR-SPACE)|A.19]] predicate basis, claim scope, selected [[A_Kernel Architecture Cluster/02_Role Taxonomy/06_A.02.06 - Unified Scope Mechanism (USM)- Context Slices & Scopes|A.2.6]] context slices, reference plane, and evaluation window across the basis and selection; (vi) coherent CN-Spec and CG-Spec editions; and (vii) satisfied admission, acceptance, and effective MinimalEvidence predicates under their direct owners.
...
* **Neighboring dated work, operation application, result binding, and evidence relations:**
  ...
  * one `U.ClaimScope`, selected [[A_Kernel Architecture Cluster/02_Role Taxonomy/06_A.02.06 - Unified Scope Mechanism (USM)- Context Slices & Scopes|A.2.6]] context slices, [[A_Kernel Architecture Cluster/19_CharacteristicSpace & Dynamics Hook (A.CHR-SPACE)/00_A.19 - CharacteristicSpace & Dynamics Hook (A.CHR-SPACE)|A.19]] predicate basis, effective reference scheme and plane, and evaluation point or interval shared as required by the selection conditions;
  ...
  * the selected-set result and any current [[F_The Unification Suite (U-Suite)- Concept-Sets, SenseCells & Contextual Role Assignment/09_Alignment and Bridge across Contexts/00_F.09 - Alignment and Bridge across Contexts|F.9]] bridge, CL, and ReferencePlane refs.
...
* The dated selection `U.Work`, actual `Select` application, finite CPM application basis, evidence-policy and `SelectionSlot` bindings, and [[A_Kernel Architecture Cluster/10_Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph/00_A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]] evidence-provenance path preserve why the reduced-set branch proceeded and why the abstain branch did not.
...
* The dated selection `U.Work`, actual `Select` application with its `TaskSignatureRef.edition` and `SelectionSlot` bindings, and [[A_Kernel Architecture Cluster/10_Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph/00_A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]] evidence-provenance path support later explanation without embedding tool tokens into the kernel.
...
* **Practice bias.** Bias against embedding telemetry and publication into selection. Risk: teams want one step to select and report. Mitigation: keep those relations under their governing patterns; retain replay through dated selection work, the actual `Select` application and result binding, [[A_Kernel Architecture Cluster/10_Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph/00_A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]] evidence provenance, and [[G_Discipline SoTA Patterns Kit/12_11_Telemetry-Driven Refresh and Decay Orchestrator/00_G.11 - Telemetry-Driven Refresh and Decay Orchestrator|G.11]] currentness.
...
| **CC-A19SelectorMechanism-13** | **Selection-scope completeness:** every actual application binds candidate universe, finite exact binary CPM application basis, required comparison coverage, token-to-producer trace, `U.ClaimScope`, selected [[A_Kernel Architecture Cluster/02_Role Taxonomy/06_A.02.06 - Unified Scope Mechanism (USM)- Context Slices & Scopes]] context slices, [[A_Kernel Architecture Cluster/19_CharacteristicSpace & Dynamics Hook (A.CHR-SPACE)/00_A.19 - CharacteristicSpace & Dynamics Hook (A.CHR-SPACE)]] predicate basis, effective reference scheme and plane, and explicit evaluation point or interval. No generic context input, optional structure, batch result, or label supplies them. |
...
| **CC-A19SelectorMechanism-16** | **No generic result relation:** the [[A_Kernel Architecture Cluster/06_Signature Stack & Boundary Discipline/06_A.06.01 - U.Mechanism - Reusable Law-Governed Operation Declaration]] operation application binds `SelectionSlot`; [[C_Kernel Extension Specifications/00_02_Epistemic holon composition (KD-CAL)/01_C.02.01 - U.Episteme- Constitution, Empirical Grounding, and Edition Relations]] governs a durable selected-set episteme when needed; direct subject patterns govern other result relations. |
...
### [[A_Kernel Architecture Cluster/19_CharacteristicSpace & Dynamics Hook (A.CHR-SPACE)/12_A.19.SelectorMechanism - Unified Selection Kernel, SelectorMechanism|A.19.SelectorMechanism]]:11.2 - Currentness and smallest reopen rule

**Qualification basis and window.** The stable kernel claim is qualified by the current editions of [[A_Kernel Architecture Cluster/06_Signature Stack & Boundary Discipline/00_A.06 - Signature Stack & Boundary Discipline|A.6]].1/A.6.5 operation and slot discipline, [[A_Kernel Architecture Cluster/19_CharacteristicSpace & Dynamics Hook (A.CHR-SPACE)/11_A.19.CPM - Unified Comparison Mechanism (CPM)|A.19.CPM]] binary application and output semantics, [[A_Kernel Architecture Cluster/19_CharacteristicSpace & Dynamics Hook (A.CHR-SPACE)/05_A.19.CN - CN-frame (comparability & normalization)|A.19.CN]] and [[G_Discipline SoTA Patterns Kit/01_00_Frame Standard and Comparability Governance - CG-Spec/00_G.0 - Frame Standard and Comparability Governance - CG-Spec|G.0]] admission and evidence rules, [[G_Discipline SoTA Patterns Kit/06_05_Multi-Method Dispatcher and MethodFamily Registry/00_G.05 - Multi-Method Dispatcher and MethodFamily Registry|G.5]] selector-policy discipline, [[A_Kernel Architecture Cluster/02_Role Taxonomy/06_A.02.06 - Unified Scope Mechanism (USM)- Context Slices & Scopes|A.2.6]] scope semantics, and the exact current [[G_Discipline SoTA Patterns Kit/03_02_SoTA Harvester & Synthesis/00_G.02 - SoTA Harvester & Synthesis|G.2]] selector pack or claim sheet cited by an actual use. For that use, the effective qualification window is the intersection of those bound editions' currentness and any validity interval declared by the selector pack, TaskSignature, or policy; `post-2015+` is an orientation label, not an indefinite freshness claim.

**Reopen the SelectorMechanism kernel only when.** Reopen the smallest affected selector rule when a direct governor changes set-return semantics, inherited SlotKinds or specialization constraints, criteria or policy binding, tri-state eligibility, the finite CPM application-basis and token-provenance boundary, selection scope, or the separation of selected set, evidence, provenance, result episteme, and publication, or when qualified evidence contradicts one of those commitments. A new selection algorithm, archive or diversity method, candidate-generation method, tie-breaker, `PortfolioMode`, rejection calibration, or domain policy that still satisfies those commitments changes its [[G_Discipline SoTA Patterns Kit/03_02_SoTA Harvester & Synthesis/00_G.02 - SoTA Harvester & Synthesis|G.2]] pack, [[G_Discipline SoTA Patterns Kit/06_05_Multi-Method Dispatcher and MethodFamily Registry/00_G.05 - Multi-Method Dispatcher and MethodFamily Registry|G.5]] policy, `CriteriaSlot`, `TaskSignature`, or other direct policy binding rather than this kernel.
...
* **Builds on**
  ...
  * [[A_Kernel Architecture Cluster/06_Signature Stack & Boundary Discipline/06_A.06.01 - U.Mechanism - Reusable Law-Governed Operation Declaration|A.6.1]] and its conformance checklist for mechanism identity, declaration content, applicability, and specialisation-chain discipline.
  ...
  * [[A_Kernel Architecture Cluster/19_CharacteristicSpace & Dynamics Hook (A.CHR-SPACE)/00_A.19 - CharacteristicSpace & Dynamics Hook (A.CHR-SPACE)|A.19]] for the exact `CharacteristicSpacePredicate` basis when one governs selection.
  * [[A_Kernel Architecture Cluster/19_CharacteristicSpace & Dynamics Hook (A.CHR-SPACE)/11_A.19.CPM - Unified Comparison Mechanism (CPM)|A.19.CPM]] for every exact binary `Compare` application in the finite basis, its pair, realized eligibility value, and own set-valued output binding.
  ...
  * [[C_Kernel Extension Specifications/10_22_Task Typing and TaskSignature Assignment (Problem-CHR)/00_C.22 - Task Typing and TaskSignature Assignment (Problem-CHR)|C.22]] for `TaskSignature` as a policy-reference artifact when used.
  * [[A_Kernel Architecture Cluster/06_Signature Stack & Boundary Discipline/16_A.06.05 - Relation-Declaration Slot Discipline - SlotKind, ValueKind, RefKind, and participant-designation discipline|A.6.5]] for slot discipline (SlotIndex as projection; SlotKind invariance).
  * [[A_Kernel Architecture Cluster/15_Role-Method-Work Alignment/03_A.15.03 - SlotFillingsPlanItem|A.15.3]] + `A.19.CHR:4.7.2` for planned slot fillings.
  * [[C_Kernel Extension Specifications/15_27_Temporal Claim Adequacy- State Readings, Temporal Trends, and Intervention-Sensitive Temporal Change/01_C.27.TA - Temporal Aspect- Time Windows, Rhythm, Cadence, and Currentness|C.27.TA]] for the explicit selection-evaluation point or interval.
  * [[A_Kernel Architecture Cluster/02_Role Taxonomy/04_A.02.04 - Episteme Evidence-Use and Status-Use Relations|A.2.4]], [[A_Kernel Architecture Cluster/10_Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph/00_A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]], and [[G_Discipline SoTA Patterns Kit/12_11_Telemetry-Driven Refresh and Decay Orchestrator/00_G.11 - Telemetry-Driven Refresh and Decay Orchestrator|G.11]] for evidence-use scope, provenance, and currentness, separately from selection scope and output.
* **Used by**
  ...
  * [[E_The FPF Constitution and Authoring Guides/17_18_Transformation Flow Structure/00_E.18 - Transformation Flow Structure|E.18]] when selector instances are used as transformation-flow structure nodes; planned refs remain `SlotFillingsPlanItem` values, while dated selection work binds effective refs and cites its direct result and evidence-provenance relations.
* **Coordinates with**
