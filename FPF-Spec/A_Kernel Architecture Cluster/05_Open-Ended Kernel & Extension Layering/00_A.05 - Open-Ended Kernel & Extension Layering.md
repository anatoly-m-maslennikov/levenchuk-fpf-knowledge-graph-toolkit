...
2. **Boundary packaging via `U.Signature` ([[A_Kernel Architecture Cluster/06_Signature Stack & Boundary Discipline/05_A.06.00 - U.Signature - Reusable Law-Governed Declaration Episteme|A.6.0]]).** Reusable bundles are published as signatures with an explicit `SignatureManifest` (`imports`, `provides`).
3. **Dependency vs specialisation are separate relations.** `imports` forms a dependency DAG constrained by **[[E_The FPF Constitution and Authoring Guides/04_05_Four Guard-Rails of FPF/03_E.05.03 - Unidirectional Dependency|E.5.3]]**; refinement/extension (`⊑`, `⊑⁺`) is expressed separately (e.g., **[[A_Kernel Architecture Cluster/06_Signature Stack & Boundary Discipline/06_A.06.01 - U.Mechanism - Reusable Law-Governed Operation Declaration|A.6.1]] `U.MechMorph`**) and should not be conflated with `imports`.
