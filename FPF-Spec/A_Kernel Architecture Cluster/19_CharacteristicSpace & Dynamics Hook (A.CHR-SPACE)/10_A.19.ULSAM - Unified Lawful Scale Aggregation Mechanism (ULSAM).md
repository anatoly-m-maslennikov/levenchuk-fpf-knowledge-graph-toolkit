...
[[A_Kernel Architecture Cluster/06_Signature Stack & Boundary Discipline/06_A.06.01 - U.Mechanism - Reusable Law-Governed Operation Declaration|A.6.1]] governs the **template** of `U.Mechanism.Intension` and the `U.MechAuthoring` discipline; this pattern governs the **ULSAM-specific** slots, operations, laws, admissibility, and audit obligations for that template.
...
* **P2W seam:** edition/policy binding for `ΓFoldRef` / `MinimalEvidenceRef` is selected in planned baseline ([[A_Kernel Architecture Cluster/15_Role-Method-Work Alignment/03_A.15.03 - SlotFillingsPlanItem|A.15.3]] + CHR P2W hook), not invented at run time.
...
| ID | Requirement |
|----|-------------|
| **CC‑A19ULSAM‑0** | **MechAuthoring discipline:** the canonical ULSAM Mechanism.Intension in `A.19.ULSAM:4.1` MUST satisfy [[A_Kernel Architecture Cluster/06_Signature Stack & Boundary Discipline/06_A.06.01 - U.Mechanism - Reusable Law-Governed Operation Declaration]] `U.MechAuthoring` and the relevant `CC‑UM.*` checks; this pattern does not override the `U.Mechanism.Intension` shape. |
...
| **CC‑A19ULSAM‑9** | **P2W separation:** planned baseline plan items MUST bind `ΓFoldRef`/`MinimalEvidenceRef`/editions ([[A_Kernel Architecture Cluster/15_Role-Method-Work Alignment/03_A.15.03 - SlotFillingsPlanItem]] + CHR P2W hook); these bindings MUST NOT be invented as run-time decisions inside the suite protocol. |
...
- [[A_Kernel Architecture Cluster/06_Signature Stack & Boundary Discipline/06_A.06.01 - U.Mechanism - Reusable Law-Governed Operation Declaration|A.6.1]] (`U.Mechanism.Intension` shape; `U.MechAuthoring`; CC‑UM discipline).
- [[A_Kernel Architecture Cluster/06_Signature Stack & Boundary Discipline/16_A.06.05 - Relation-Declaration Slot Discipline - SlotKind, ValueKind, RefKind, and participant-designation discipline|A.6.5]] (slot discipline; SlotIndex as a projection).
