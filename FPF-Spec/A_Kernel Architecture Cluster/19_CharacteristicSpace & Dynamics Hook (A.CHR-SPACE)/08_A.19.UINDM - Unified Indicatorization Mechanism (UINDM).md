...
> [[A_Kernel Architecture Cluster/06_Signature Stack & Boundary Discipline/06_A.06.01 - U.Mechanism - Reusable Law-Governed Operation Declaration|A.6.1]] governs the **template** of `U.Mechanism.Intension`.
>
...
* **P2W seam:** concrete edition/policy pins are bound in planned baseline plan items ([[A_Kernel Architecture Cluster/15_Role-Method-Work Alignment/03_A.15.03 - SlotFillingsPlanItem|A.15.3]] + `A.19.CHR:4.7.2`); executions only record effective refs/pins in `Audit`.
