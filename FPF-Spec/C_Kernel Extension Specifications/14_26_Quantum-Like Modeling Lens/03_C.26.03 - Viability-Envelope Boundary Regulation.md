...
| Mini-entry | Question |
| --- | --- |
| Viability bearer | Which exact object fills this local lens position, and which direct identity, relation, or structure owner establishes it? If it is a system, where is the [[A_Kernel Architecture Cluster/01_Holon Ontic Foundation (U.Holon and Admitted Holon Kinds)/00_A.01 - Holon Ontic Foundation (U.Holon and Admitted Holon Kinds)]] basis; if a selected role configuration or structure, which exact relation or structure selects it; if a population or market slice, what declares its domain/reference scheme, membership or scope, and identity basis? |
...
- the local viability-bearer position points to one exact object already identified under its direct owner and introduces no kind or relation; a system reading has an [[A_Kernel Architecture Cluster/01_Holon Ontic Foundation (U.Holon and Admitted Holon Kinds)/00_A.01 - Holon Ontic Foundation (U.Holon and Admitted Holon Kinds)|A.1]] basis, a selected role configuration or structure has its exact relation or structure owner, and a population or market slice has a declared domain/reference scheme, membership or scope, and identity basis;
...
### [[C_Kernel Extension Specifications/14_26_Quantum-Like Modeling Lens/03_C.26.03 - Viability-Envelope Boundary Regulation|C.26.3]]:4.7 - Sensor, probe, candidate-intervention, and metric split
...
The pattern may feel too broad if it is applied to every quality concern. It is not for every quality concern. Use [[C_Kernel Extension Specifications/13_25_Q-Bundle- Authoring --ilities- as Structured Quality Bundles/00_C.25 - Q-Bundle- Authoring --ilities- as Structured Quality Bundles|C.25]] alone when one quality bundle or metric can be handled without envelope, disturbance, boundary condition, recovered candidate intervention, adaptation cost, or viability failure mode.
