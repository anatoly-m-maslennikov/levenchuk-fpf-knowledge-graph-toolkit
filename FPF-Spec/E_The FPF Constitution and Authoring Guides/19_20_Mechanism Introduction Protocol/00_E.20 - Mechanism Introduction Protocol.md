...
2. **New mechanism definition within an existing [[A_Kernel Architecture Cluster/06_Signature Stack & Boundary Discipline/06_A.06.01 - U.Mechanism - Reusable Law-Governed Operation Declaration|A.6.1]] mechanism kind** (new `MechanismDefinitionRef`, new canonical card).
...
| Planned baseline pins (planned slot fillings, edition-pinned refs, explicit time selector) | **WorkPlanning governing pattern** | [[A_Kernel Architecture Cluster/15_Role-Method-Work Alignment/03_A.15.03 - SlotFillingsPlanItem]] plus suite-specific specialization when needed | SHALL NOT embed launch values, witnesses, or gate decisions in planning. |
...
| --- | --- | --- | --- |
| Mechanism semantics in [[A_Kernel Architecture Cluster/06_Signature Stack & Boundary Discipline/06_A.06.01 - U.Mechanism - Reusable Law-Governed Operation Declaration]], effects-handler practice, and refinement-style signature discipline require an explicit operation/signature/law/admissibility locus. | Mechanism meaning is assigned to A.6.1-governed mechanism definitions: operation algebra, law set, admissibility conditions, `SlotIndex`, required input/output `SlotKind`s, per-operation `SlotSpec`s, transport/bridge regime, applicability, audit, and monotone realization relation when declared. | When a mechanism is introduced or changed, name the mechanism-governing definition that carries those semantic fields before suites, plans, or wiring cite it. | Treating suite text, wiring prose, or a MIP manifest as mechanism semantics. |
...
**Coordinates with:**
* **[[A_Kernel Architecture Cluster/06_Signature Stack & Boundary Discipline/06_A.06.01 - U.Mechanism - Reusable Law-Governed Operation Declaration|A.6.1]]** (`U.Mechanism` definition template governance)
* **[[A_Kernel Architecture Cluster/06_Signature Stack & Boundary Discipline/18_A.06.07 - MechSuiteDescription - Description of a set of distinct mechanisms|A.6.7]]** (`MechSuiteDescription` integrity)
* **[[A_Kernel Architecture Cluster/15_Role-Method-Work Alignment/03_A.15.03 - SlotFillingsPlanItem|A.15.3]]** (`SlotFillingsPlanItem` and planned baseline seam)
