...
* **ConformanceRules (examples).**
  ...
  * Functional ports and signatures are separated from module interfaces unless an [[A_Kernel Architecture Cluster/06_Signature Stack & Boundary Discipline/15_A.06.M - Module Relation Repair|A.6.M]] module-interface claim is current.
...
* **AllowedEpistemeKinds (shape).**
  ...
  * `WorkflowDescription`, `WorkflowSpec` (business processes, orchestration logic).
  ...
  These epistemes:
  * respect the **order discipline** (Γ_method, Γ_ctx) and [[A_Kernel Architecture Cluster/15_Role-Method-Work Alignment/00_A.15 - Role-Method-Work Alignment|A.15]] (Role–Method–Work alignment);
  * carry E.10.D2-conformant DescriptionContext with `ViewpointRef = VP.Procedural`.

* **ConformanceRules (examples).**
  * Preconditions and postconditions at step boundaries are explicit and type-checked. Use [[A_Kernel Architecture Cluster/03_Transformer Constitution (Quartet)/01_A.03.01 - U.Method- Reusable Way of Doing with Explicit Applicability|A.3.1]] for the method as semantic way of doing, [[A_Kernel Architecture Cluster/03_Transformer Constitution (Quartet)/02_A.03.02 - U.MethodDescription- Description Episteme for a Way of Doing|A.3.2]] for the method description, and `Gamma_method` only as notation over the recovered method claim.
...
* **ConformanceRules (examples).**
  * Role vs method vs work vs capability separation is upheld through [[A_Kernel Architecture Cluster/07_Strict Distinction (Clarity Lattice)/00_A.07 - Strict Distinction (Clarity Lattice)|A.7]] and [[A_Kernel Architecture Cluster/15_Role-Method-Work Alignment/00_A.15 - Role-Method-Work Alignment|A.15]].
...
* **Coordinates with.** MVPK publication face, publication form, publication unit, and publication carrier discipline; [[A_Kernel Architecture Cluster/02_Role Taxonomy/00_A.02 - Role Taxonomy|A.2]], [[A_Kernel Architecture Cluster/02_Role Taxonomy/01_A.02.01 - U.RoleAssignment - System Role Assignment|A.2.1]], [[A_Kernel Architecture Cluster/15_Role-Method-Work Alignment/00_A.15 - Role-Method-Work Alignment|A.15]], and Part F role-description and role-name patterns; [[F_The Unification Suite (U-Suite)- Concept-Sets, SenseCells & Contextual Role Assignment/18_Local-First Unification Naming Protocol/00_F.18 - Local-First Unification Naming Protocol|F.18]] (naming discipline for ViewFamilyId, ViewpointId, EngineeringVPId, and publication-side viewpoint ids).
