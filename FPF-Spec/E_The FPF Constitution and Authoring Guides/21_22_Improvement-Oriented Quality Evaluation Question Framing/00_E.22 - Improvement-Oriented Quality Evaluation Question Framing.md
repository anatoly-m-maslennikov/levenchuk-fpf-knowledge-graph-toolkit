...
Here *move* is Plain wording for writing the frame. It is not a shared Move identity, selected repair, WorkPlan, performed `U.Work`, or actual `U.Transformation`; if dated framing work itself matters, [[A_Kernel Architecture Cluster/15_Role-Method-Work Alignment/00_A.15 - Role-Method-Work Alignment|A.15]] governs that separate occurrence.
...
A below-floor value, finding, improvement aim, or need for evaluation is not by itself an actual Problem. If the consuming use relies on an actual Problem, cite one current [[C_Kernel Extension Specifications/10_22_Task Typing and TaskSignature Assignment (Problem-CHR)/02_C.22.PFR - Problematic-For Relation|C.22.PFR]] `ProblematicForRelation` occurrence with its direct participants and temporal identity; the frame, evaluation, result, and evidence may support a claim about it but neither create nor split it.
...
The remaining local support names ending in `@Context` are compatibility and retrieval names only. The suffix supplies no context entity, scope, participant, relation, or identity component; every episteme follows [[C_Kernel Extension Specifications/00_02_Epistemic holon composition (KD-CAL)/01_C.02.01 - U.Episteme- Constitution, Empirical Grounding, and Edition Relations|C.2.1]] identity, every set is identified by its stated extensional rule, and every neighboring Work, decision, evidence, viewpoint, grounding, or result relation remains under its direct governor.
...
`resultConsumingUseRef` is not a generic *use* placeholder. Before occurrence it may resolve to one [[A_Kernel Architecture Cluster/15_Role-Method-Work Alignment/02_A.15.02 - U.WorkPlan|A.15.2]] `U.WorkPlan` that names the particular intended Work, or to the exact decision question or decision-governing object under its direct pattern. It may resolve to `U.Work` only when that dated Work already obtains under [[A_Kernel Architecture Cluster/15_Role-Method-Work Alignment/01_A.15.01 - U.Work|A.15.1]]. The frame neither creates the consuming Work or decision nor authorizes it.
...
**Floor evaluation.** A reviewer is asked whether one pattern is ready for ordinary use. The frame names the exact pattern version, [[E_The FPF Constitution and Authoring Guides/20_21_FPF Pattern-Quality Evaluation CharacteristicSpace/00_E.21 - FPF Pattern-Quality Evaluation CharacteristicSpace|E.21]] characteristic space and floor predicate, the evaluation ClaimScope, the decision that will consume the result, [[E_The FPF Constitution and Authoring Guides/20_21_FPF Pattern-Quality Evaluation CharacteristicSpace/00_E.21 - FPF Pattern-Quality Evaluation CharacteristicSpace|E.21]] as the governing evaluation pattern, purpose `floorEvaluation`, the declared floor, and the expected [[E_The FPF Constitution and Authoring Guides/20_21_FPF Pattern-Quality Evaluation CharacteristicSpace/00_E.21 - FPF Pattern-Quality Evaluation CharacteristicSpace|E.21]] result form. Any semantic evaluation Method is named separately when it is recoverable. The result becomes current only through the direct [[E_The FPF Constitution and Authoring Guides/20_21_FPF Pattern-Quality Evaluation CharacteristicSpace/00_E.21 - FPF Pattern-Quality Evaluation CharacteristicSpace|E.21]] evaluation under one dated evaluation Work and its exact typed result relation or binding: a complete [[E_The FPF Constitution and Authoring Guides/20_21_FPF Pattern-Quality Evaluation CharacteristicSpace/00_E.21 - FPF Pattern-Quality Evaluation CharacteristicSpace|E.21]] coordinate table with `ShortRationale` and `EvaluationEvidenceBasis`, not a narrative "looks fine" and not the frame itself.
...
| `CC-E22-10` | Keep the question frame, use declaration, governing evaluation pattern, optional semantic Method, any independently admitted method description actually cited, quality-model/evidence/result-form descriptions, performer assignment, dated evaluation Work, method enactment, any actual operation application, actual evidence use, typed result binding or direct result relation, optional result episteme, and result-consuming work or decision distinct. A governing-pattern reference does not establish [[A_Kernel Architecture Cluster/03_Transformer Constitution (Quartet)/02_A.03.02 - U.MethodDescription- Description Episteme for a Way of Doing]] membership, and no generic work-result or evaluation-result relation is inferred. |
| `CC-E22-11` | A low value, finding, failed floor, or improvement aim does not establish an actual Problem. Any actual Problem relied on by the consuming use resolves to one current [[C_Kernel Extension Specifications/10_22_Task Typing and TaskSignature Assignment (Problem-CHR)/02_C.22.PFR - Problematic-For Relation]] occurrence with its direct participants and temporal identity. |
...
| **Context-labelled frame.** Project, domain, dashboard, cadence, or context label supplies identity or evaluation scope. | Identify the frame by its [[C_Kernel Extension Specifications/00_02_Epistemic holon composition (KD-CAL)/01_C.02.01 - U.Episteme- Constitution, Empirical Grounding, and Edition Relations]] claim content and exact EntityOfConcern; bind the exact `U.ClaimScope` and other use values separately. |
...
| **Frame replaces result.** The question frame names a purpose but returns prose, a two-column value table, or proposal rows without the named evaluation's result form. | Re-run the named evaluation as dated Work with its direct method enactment or [[A_Kernel Architecture Cluster/06_Signature Stack & Boundary Discipline/06_A.06.01 - U.Mechanism - Reusable Law-Governed Operation Declaration]] operation application and return its declared coordinates, evidence basis, rationales, payload fields, and typed result binding or direct result relation. |
| **Description performs evaluation.** A method description, characteristic-space specification, expected evidence basis, role assignment, or result-form description is treated as evaluation Work or its result. | Keep each description and assignment separate; identify one dated [[A_Kernel Architecture Cluster/15_Role-Method-Work Alignment/01_A.15.01 - U.Work]] Work occurrence, the direct evaluation application, actual evidence use, and result under their exact governors. |
...
| **Finding as actual Problem.** A low coordinate, finding, or floor miss is treated as a Problem occurrence. | Keep the evaluation result epistemic; cite [[C_Kernel Extension Specifications/10_22_Task Typing and TaskSignature Assignment (Problem-CHR)/02_C.22.PFR - Problematic-For Relation]] only when its actual-condition and criterion-applicability participants make one ProblematicFor occurrence obtain. |
...
| [[A_Kernel Architecture Cluster/19_CharacteristicSpace & Dynamics Hook (A.CHR-SPACE)/00_A.19 - CharacteristicSpace & Dynamics Hook (A.CHR-SPACE)]], [[A_Kernel Architecture Cluster/19_CharacteristicSpace & Dynamics Hook (A.CHR-SPACE)/01_A.19.ECS - Evaluation CharacteristicSpace Construction]], [[A_Kernel Architecture Cluster/19_CharacteristicSpace & Dynamics Hook (A.CHR-SPACE)/11_A.19.CPM - Unified Comparison Mechanism (CPM)]], [[A_Kernel Architecture Cluster/02_Role Taxonomy/06_A.02.06 - Unified Scope Mechanism (USM)- Context Slices & Scopes]] | Govern the selected `CharacteristicSpace`, its construction description, predicate/comparator semantics and actual comparison application, and exact `U.ClaimScope`; [[E_The FPF Constitution and Authoring Guides/21_22_Improvement-Oriented Quality Evaluation Question Framing/00_E.22 - Improvement-Oriented Quality Evaluation Question Framing]] binds their values for one question but does not redefine them. |
| [[A_Kernel Architecture Cluster/15_Role-Method-Work Alignment/01_A.15.01 - U.Work]], [[A_Kernel Architecture Cluster/06_Signature Stack & Boundary Discipline/06_A.06.01 - U.Mechanism - Reusable Law-Governed Operation Declaration]], [[C_Kernel Extension Specifications/00_02_Epistemic holon composition (KD-CAL)/01_C.02.01 - U.Episteme- Constitution, Empirical Grounding, and Edition Relations]] | Govern dated evaluation Work and method enactment, any actual operation application and result binding, and any durable result episteme. The direct evaluation pattern owns its typed result relation; [[E_The FPF Constitution and Authoring Guides/21_22_Improvement-Oriented Quality Evaluation Question Framing/00_E.22 - Improvement-Oriented Quality Evaluation Question Framing]] mints no generic evaluation-result or work-result relation. |
| [[A_Kernel Architecture Cluster/02_Role Taxonomy/04_A.02.04 - Episteme Evidence-Use and Status-Use Relations]], [[A_Kernel Architecture Cluster/10_Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph/00_A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph]], [[G_Discipline SoTA Patterns Kit/12_11_Telemetry-Driven Refresh and Decay Orchestrator/00_G.11 - Telemetry-Driven Refresh and Decay Orchestrator]] | Govern actual evidence use, provenance, and currentness separately from the expected evidence-basis description. |
| [[C_Kernel Extension Specifications/10_22_Task Typing and TaskSignature Assignment (Problem-CHR)/02_C.22.PFR - Problematic-For Relation]] | Governs an actual Problem occurrence when the consuming use relies on one; evaluation need, finding, or floor failure alone establishes none. |
